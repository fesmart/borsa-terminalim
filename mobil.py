...
# --- 🔐 GÜVENLİ ŞİFRE YÖNETİMİ ---
# Artık şifreyi kodun içine yazmıyoruz, Streamlit'in gizli kasasından çekiyoruz.
try:
    API_KEY = st.secrets["GEMINI_API_KEY"]
except:
    st.error("⚠️ API Anahtarı bulunamadı! Lütfen Streamlit ayarlarından Secrets kısmını doldurun.")
    st.stop()
...
arama_kutusu = st.text_input("🔍 Yeni bir hisse keşfet (Örn: TUPRS):")
...
def analiz_motoru(hisseler, portfoy_modu=False):
    ...
    for i, hisse in enumerate(hisseler):
        try:
            ...
            veri = {
                'Hisse': hisse.replace('.IS', ''), 'Fiyat': round(fiyat, 2),
                'SMA50': round(sma50, 2) if isinstance(sma50, float) else sma50,
                'MC_Tahmin': round(mc_beklenti, 2), 'F/K': round(fk, 2) if isinstance(fk, float) else fk,
    ...
    if st.button("🔍 Bireysel Analiz", use_container_width=True):
        if arama_kutusu:
            ...
            if veriler:
                ...
                st.dataframe(df)
                prompt = f"Şu hisseleri bağımsız bir temel analiz uzmanı gibi detaylı incele. F/K, ROE ve Monte Carlo verilerine odaklan:\n{df.to_string(index=False)}"
                try:
                    client = genai.Client(api_key=API_KEY)
                    res = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
                    st.success("🧠 Analiz Raporu:")
                    st.write(res.text)
...
with col2:
    if st.button("💼 Portföyümü İncele", use_container_width=True):
        if st.session_state['portfoy']:
            ...
            if veriler:
                df = pd.DataFrame(veriler)
                st.metric(label="💰 Toplam Cüzdan", value=f"{round(top_deger, 2)}", delta=f"{round(top_kar, 2)}")
                st.dataframe(df)
                prompt = f"Sen bir fon yöneticisisin. Portföyümü analiz et:\n{df.to_string(index=False)}"
                try:
                    client = genai.Client(api_key=API_KEY)
                    res = client.models.generate_content(model='gemini-2.5-flash', contents=prompt)
                    st.info("🧠 Fon Raporu:")
